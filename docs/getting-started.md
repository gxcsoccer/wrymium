...
首次编译较慢（下载 CEF + CMake 编译 libcef_dll_wrapper）。后续编译正常速度。

### 5. 打包 macOS .app

CEF 在 macOS 上需要 `.app` bundle 结构（含 framework + helper apps）。wrymium 提供了打包脚本：

```bash
# 克隆 wrymium 仓库获取打包脚本
git clone https://github.com/gxcsoccer/wrymium.git /tmp/wrymium

# 打包（假设你的 binary 名称是 my-cef-app）
bash /tmp/wrymium/scripts/bundle-macos.sh my-cef-app "My CEF App"

# 运行
open target/bundle/my-cef-app.app
```
...
# 用 wrymium 打包脚本创建 .app bundle
bash /path/to/wrymium/scripts/bundle-macos.sh your-app-name "Your App"

# 运行
open target/bundle/your-app-name.app
```
...
macOS 上 CEF 需要 `.app` bundle 结构。不能直接 `cargo run`，必须用打包脚本生成 `.app` 再用 `open` 启动。
...
| 不能 `cargo run` | macOS 需要 .app bundle |
